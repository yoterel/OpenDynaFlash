...
public:
    DynaFlashProjector(bool flip_ver = false, bool flip_hor = false, int queue_length = 20, bool verbose = false) : m_projector_queue(queue_length),
                                                                                                                    ...
                                                                                                                    m_flip_hor(flip_hor),
                                                                                                                    m_verbose(verbose),
                                                                                                                    m_white_frame(m_frame_size, 255){};
    ~DynaFlashProjector()
    {
        gracefully_close();
    };
    bool init();
    void show();
    bool show_buffer(uint8_t *buffer, bool blocking = true);
    ...
    void post_internal_buffer();
    std::size_t get_queue_size() { return m_projector_queue.size_approx(); };

#ifdef PYTHON_BINDINGS_BUILD
    bool project(nb::ndarray<uint8_t, nb::shape<768, 1024, 3>,
                             nb::c_contig, nb::device::cpu>
                     data)
    {
        return show_buffer(data.data());
    };
#endif
private:
    void print_version();
    void print_brightness_values();
    void set_brightness_values();
    const int m_frame_height = 768;
    const int m_frame_width = 1024;
    std::vector<uint8_t> m_white_frame;
